...
class SqliteDocumentRepository:
    ...
    def recover_abandoned_indexing(self, owner_id: str | None = None) -> list[str]:
        ...
        - **기동 시 1회** — `owner_id`를 넘긴다. 이전 자신이 남긴 것을 회수한다
        - **주기 호출** — 인자 없이 부른다. 리스가 만료된 것만 회수한다

        주기 호출에 자기 `owner_id`를 넘기면 진행 중인 자기 색인을 강등한다.
        그 뒤 heartbeat과 mark_ready가 소유자 불일치로 거부되어 색인이 끝나지 않는다.
        ...
        호출 방식을 잘못 쓸 여지를 없애려면 메서드를 둘로 나누는 편이 확실하다.
        색인 서비스를 만들 때 실제 호출 형태를 보고 정한다.
        """
