...
# 바꾸면 NOT_GROUNDED 판정 분포가 통째로 바뀌므로 버전을 올리고 기록한다(DP-56).
# 변경 전후는 scripts/probe_answers.py로 같은 질문을 돌려 비교한다.
PROMPT_VERSION = "v2"

# v2에서 더한 것: 항목 확인, 조건 분기, 확인 불가 시 거절 강화.
# 실물에서 "정격출력이 얼마에요?"에 정격 **전류**를 답한 사례가 나왔다.
# 사양표가 표 구조를 잃고 한 줄 텍스트로 추출되어 항목과 값의 대응이 끊긴
# 탓이다(2026-09-07 측정). 파서를 고치기 전까지는 모델이 스스로 항목
# 일치를 확인하도록 지시한다.
_PROMPT_TEMPLATE = """다음 근거만 사용해 질문에 답하라. 근거에 없는 내용은 답하지 마라.

지킬 것:
1. 값을 답하기 전에 **그 값이 질문한 항목의 값이 맞는지** 확인하라. 근거에 있는
   표는 항목과 값이 떨어져 있을 수 있다. 항목 이름이 다르면(예: 출력을 물었는데
   전류 값만 있는 경우) 그 값을 답하지 말고 확인할 수 없다고 하라.
2. 값이 조건(모델·용량·단자 종류 등)에 따라 여러 개면 하나로 고르지 말고
   조건과 함께 모두 제시하라.
3. 답변에 사용한 근거의 번호를 문장 끝에 [1] 형태로 반드시 표기하라.
4. 근거에서 답을 찾을 수 없거나 항목이 일치하는지 확신할 수 없으면
   "근거 자료에서 확인할 수 없습니다"라고만 답하라. 추측하지 마라.
