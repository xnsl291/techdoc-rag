...
def fetch_health(base_url: str, timeout_seconds: float = 5.0) -> dict:
    ...
    try:
        ...
        # 503의 detail 안에 같은 모양(status/components)이 들어 있다.
...
def _read_detail(error: urllib.error.HTTPError) -> str:
    detail = _read_json(error).get("detail", "")
    return str(detail)[:300] if detail else "상세 없음"
