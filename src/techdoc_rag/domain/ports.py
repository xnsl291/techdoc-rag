...
class EmbeddingModel(Protocol):
    ...
    **입력을 배치로 나눠 처리한다.** 실물 말뭉치가 2,454청크이고 청크가 1,200자
    안팎이다. 한 번에 넘기면 모델 쪽 메모리를 통제할 수 없다. 배치 크기는
    구현체가 정하고, 호출자는 전량을 넘긴다.

    **HTTP로 접속하는 구현체는 커넥션을 재사용한다.** 요청마다 새로 열면
    Windows에서 TIME_WAIT가 쌓여 동적 포트가 고갈된다. 그리고 localhost는
    IPv6로 먼저 해석되는데 서버가 IPv4에만 바인딩되어 있으면 폴백이 일어나
    요청당 2초가 붙는다. 2026-08-30 실측에서 2,162ms 대 86ms였다.

    **max_input_tokens를 넘는 입력은 조용히 잘린다.** 모델이 실제로 받는 길이를
    알려야 색인 서비스가 청크 길이를 확인할 수 있다. 태그에 적힌 값과 실행 시점에
    로드되는 값이 다를 수 있으므로 구현체가 실제 값을 돌려준다.
...
class LlmClient(Protocol):
    ...
    **HTTP 커넥션을 재사용한다.** 요청마다 새로 열면 Windows에서 TIME_WAIT가 쌓여
    동적 포트가 고갈되고, 평가 스크립트가 문항을 연속 처리할 때 실패한다.

    **동시 생성 수를 제한한다.** 설정값으로 받고 그 수를 넘는 요청은 대기시킨다.
    CPU 한 대에서 생성을 둘 돌리면 서로 CPU를 나눠 써서 총 처리량이 늘지 않는다.
    제한하지 않으면 커넥션 하나에 요청 둘이 섞여 답변이 깨진다.
    이 값은 GPU를 확보하거나 추론 서버를 분리하면 올린다(DP-51).

    **타임아웃을 건다.** 서버가 응답을 멈추면 무한정 기다리게 된다.
    대기와 생성 각각에 상한을 두고, 넘으면 GenerationError로 끊는다.

    **이터레이터를 닫으면 생성을 멈춘다.** 사용자가 화면을 닫으면 서비스가
    이터레이터를 버리는데, 그때 서버 쪽 생성이 계속 돌면 CPU를 붙잡는다.
    구현체는 GeneratorExit를 받아 연결을 정리한다(01 §17.8).
